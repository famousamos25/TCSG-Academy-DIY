...
export function SelectDisputeLetter({filterName, emitFilterName, resetIsActive}: SelectDisputeLetterProps) {
    ...
    <div className="flex flex-col border-[1px] border-gray-300 rounded-md">
      ...
      <div className="max-h-[45vh] overflow-y-auto px-4 py-0">
        {filteredData.map((l, i) => (
          <DisputeLetterCard 
          key={i} data={l} 
          emitFilterName={(name)=> handleEmittedName(name)} 
          handleClick = {() => setActiveCard(l.title)}
          isActive = {l.title === activeCard}
          />
        ))}
      </div>
      {
